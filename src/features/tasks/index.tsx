...
export const Tasks = () => {
  const selectCompletedTodos = useMemo(() => {
    return createSelector(
      ...
      (data: TasksResponse) =>
        data?.filter((task: Task) => task.completed === true) ?? []
    );
  }, []);
  ...
  const selectInCompletedTodos = useMemo(() => {
    return createSelector(
      (inputData: any) => inputData?.data,
      (data: TasksResponse) =>
        data?.filter((task: Task) => task.completed === false) ?? []
    );
  }, []);
  ...
  const {
    ...
    selectFromResult: (result) => ({
      ...result,
      allCompletedTasks: selectCompletedTodos(result),
      allIncompletedTasks: selectInCompletedTodos(result),
    }),
  });
